...
function fetchImages(){
    fetch('images.json') // Path to your JSON file
            ...
            .then(data => {
                imgData = data;
                imgKey = Object.keys(data);
                console.log(imgKey);
            })
            .catch(error => console.log('Error:', error));
}

function displayImageData(x,y,ind){
    console.log(imgData[ind]);
    const text = document.createElement('p');
    text.innerHTML = "location:" + imgData[ind]["location"] + "<br>" + "system:" + imgData[ind]["system"];
    text.classList.add('detail');
    text.style.left = `${x}px`;
    text.style.top = `${y}px`;
    // text.style.color = getRandomRgbColor();
    text.style.transform = `translate(50%, -70%)`; // Centers the image at the cursor
...
function createRandomImage(x, y) {
    ...
    curKey= imgKey[Math.floor(Math.random() * imgKey.length)];
    img.src = curKey;
